...
def labelLocation(ds, location):
    ...
    Input:
        ds       -  xarray dataset with DTS data. Expects to find a dimension labeled 'LAF'
        location -  dictionary specifying location labels of the form:
                    dict['label'] = [float of section LAF start, float of section LAF end]
                    If the section has a fiber that 'points'
    Output:
        ds       -  the same xarray that was passed to the function, but with the
                    'location' and 'location_flip' coordinates.
    '''
    ...
    for l in location:
        ...
        if np.size(shape) > 1:
            for loc_num in np.arange(0, max(shape)):
                ...
                # For locations where the relative start is at a larger LAF than the relative end of the section, 
                # indicate that we need to flip the LAF
                if not location[l][loc_num][0] > location[l][loc_num][-1]:
                    ds.coords['location_flip'].loc[(ds.LAF > LAF1) & (ds.LAF < LAF2)] = True
...
def dtsPhysicalCoords(ds, location, coord_opt='relative', align='right'):
    ...
    Input:
        ds            -  xarray dataset with DTS data. Expects to find a dimension labeled 'LAF'
        location_list -  List containing location labels for convertitn LAF to a physically coordinate
    Output:
        ds            -  xarray Dataset formatted to include just the physically labeled coordinate system.
    '''
    ...
    if coord_opt == 'relative':
        ...
        # Assumed that the first LAF section refers to the start of the relative section
        # and the last one is the end of the relative distance section.
        for l in location:
            shape = np.shape(location[l])

            ########
            # For non-contiguous label locations, loop through each labeled section
            if np.size(shape) > 1:
                for loc_num in np.arange(0, max(shape)):
                    LAF1 = min(location[l][loc_num])
                    LAF2 = max(location[l][loc_num])
                    ...
                    # For locations where the relative start is at a larger LAF than the relative end
                    # of the section, indicate that we need to flip the LAF
                    if not location[l][loc_num][0] > location[l][loc_num][-1]:                        
        ...
        for section_num, section in enumerate(all_sections):
            ...
            section.coords['location'] = (('section_num'), np.unique(section.location.values))
...
def yamlDict(yamlPath):
    '''
    Reads the .yml label location file and returns the location dictionary for labeling.
    INPUT:
